<template>
  <div class="absolute inset-0 flex items-center justify-center">
    ...
    <video-player
      class="video-player max-h-full"
      crossorigin="anonymous"
      controls

      :sources="[
        {
          src: lecture.video.src,
...
const handleProgress = (payload) => {
  ...
  const played = player.value?.currentTime();
  const pct_played = (played / duration) * 100;

  if (pct_played >= 70 && !isLogged.value) {
...
const handleReady = () => {
  ...
  vhs.xhr.beforeRequest = (options) => {
    // next hls track is being fetched
    options.maxPlaylistRetries = 2
    // options.headers = {
    //   'X-Video': props.lecture.video?.id
    // }   
    return options;
  };
...
.video-js .vjs-big-play-button {
  background-color: rgba(204, 24, 30, 0.46);
  font-size: 2.5em;
  border-radius: 50%;
